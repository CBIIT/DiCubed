ui <- fluidPage(
  ...
  # App title ----
  titlePanel(HTML("DI<sup>3</sup> Data Downloads")),
...
server <- function(input, output) {
 ...
 paste(
"SELECT collection, subject_id, tcia_subject_id, 
                            sex_ncit , sex_value, 
                            ...
                            from row_export_data where ", where_clause, "  order by collection, subject_id ")
 ...
 # $names(df_postgres)[1] <- 'Collection'
  colnames(df_postgres) <-  c('Collection', 'Subject ID', 'TCIA Subject ID','Sex NCIT (C28421)', 'Sex Value', 'Age NCIT (C69260)', 'Age', 'Age Units',
                              ...
                              'Anatomic Site NCIT (C13717)', 'Anatomic Site'
                              )
