ui <- fluidPage(
  #titlePanel(HTML("DI<sup>3</sup> SDTM Export Plugin")),
  sidebarLayout(
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    where_clause <- paste(" ( select pd.tcia_subject_id from 
                        di3crcdata.qt_query_result_instance qri 
    ...
    sql_string <- paste("select collection as STUDYID, cast('DM' as varchar(2))  as DOMAIN, 
                        ...
                        from di3sources.row_export_data where tcia_subject_id in ", where_clause)
    ...
    sql_string = paste( "select distinct red.collection as studyid,
                        ...
                        from di3sources.row_export_data red join di3crcdata.patient_mapping pm on red.subject_id = pm.patient_ide where red.tcia_subject_id in ", where_clause,
    ...
    sql_string <- paste( " with
                         ...
                         where  m.tcia_subject_id in ", where_clause ,  " order by m.tcia_subject_id, m.study_date ") 
    ...
    sql_string <- paste("
                        ...
                        from di3sources.row_export_data where er_value is not null and tcia_subject_id in (", where_clause , 
                        ") union 
                        select collection as studyid, cast('MI' as varchar(2)) as domain, tcia_subject_id as USUBJID, 
                        'PROGESTR' as mitestcd, 'Progesterone Receptor' as MITEST, pr_value as MIORRES,
                        cast('TISSUE' as varchar(6)) as MISPEC, cast('BREAST' as varchar(6)) as MILOC 
                        from di3sources.row_export_data where pr_value is not null and tcia_subject_id in (" , where_clause, ") 
                        union 
                        select collection as studyid, cast('MI' as varchar(2))  as domain, tcia_subject_id as USUBJID, 
                        'HER2' as mitestcd, 'Human Epidermal Growth Factor Receptor 2' as MITEST, her2_value as MIORRES,
                        cast('TISSUE' as varchar(6)) as MISPEC, cast('BREAST' as varchar(6)) as MILOC 
                        from di3sources.row_export_data where her2_value is not null and tcia_subject_id in  (", where_clause, ") 
                        ) 
    ...
    sql_string <- paste(
      ...
      from di3sources.row_export_data where course_of_disease_value is not null and tcia_subject_id  in (", where_clause, ") 
      )
    ...
    sql_string <- paste( " 
                         ...
                         where pd.tcia_subject_id in (", where_clause ,") and series.modality = 'MR'
    ...
    sql_string <- paste( "with
                         ...
                         where aud.tcia_subject_id in (", where_clause , ")
                         )
                         ,  ispy_tr 
                         as 
                         (select df.collection as studyid, 
